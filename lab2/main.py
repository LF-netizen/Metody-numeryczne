...
def parallel_plot(x1:np.ndarray,y1:np.ndarray,x2:np.ndarray,y2:np.ndarray,
    ...
    if x1.shape != y1.shape or x2.shape != y2.shape or min(x1.shape) == 0 or min(x2.shape) == 0 or (orientation != '-' and orientation != '|'):
        return None

    fig, (ax1, ax2) = plt.subplots(2 if orientation == '-' else 1, 2 if orientation == '|' else 1)
    fig.suptitle(title)
